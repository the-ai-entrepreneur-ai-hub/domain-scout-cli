...
class RobustLegalExtractor:
    ...
    def extract(self, html: str, url: str) -> Dict:
        ...
        # PASS 1: Extract from structured data (highest priority)
        structured_data = self._extract_from_structured_data(soup)
        
        # PASS 2: Section-based extraction
    ...
    def _merge_and_validate(self, structured: Dict, section: Dict, country: str) -> Dict:
        """Merge results with priority to structured data, validate all fields."""
        result = {}
        
        # Priority order: structured > section
        all_keys = set(structured.keys()) | set(section.keys())
        
        for key in all_keys:
            # Prefer structured data
            value = structured.get(key) or section.get(key)
            if value:
                result[key] = value
